...
static tegra_dc_bl_output dsi_a_768p_10_1_bl_output_measured = {
#if 1
	0,	1,	1,	2,	2,	3,	4,	4,
	5,	5,	7,	9,	10,	12,	13,	15,
	17,	18,	20,	22,	23,	23,	24,	25,
...
static struct tegra_dsi_cmd dsi_a_768p_10_1_init_cmd[] = {
	/* no init command required */
};

static struct tegra_dsi_out dsi_a_768p_10_1_pdata = {
	...
	.dsi_init_cmd = dsi_a_768p_10_1_init_cmd,
	.n_init_cmd = ARRAY_SIZE(dsi_a_768p_10_1_init_cmd),
};

static int dsi_a_768p_10_1_regulator_get(struct device *dev)
{
	int err = 0;
...
static int dsi_a_768p_10_1_gpio_get(void)
{
	int err = 0;
...
static int dsi_a_768p_10_1_postpoweron(struct device *dev)
{
	return 0;
}

static int dsi_a_768p_10_1_enable(struct device *dev)
{
	int err = 0;

	err = dsi_a_768p_10_1_regulator_get(dev);
	if (err < 0) {
		pr_err("dsi regulator get failed\n");
		goto fail;
	}

	err = dsi_a_768p_10_1_gpio_get();
	if (err < 0) {
...
static int dsi_a_768p_10_1_prepoweroff(void)
{
	return 0;
}

static int dsi_a_768p_10_1_disable(void)
{
...
static int dsi_a_768p_10_1_postsuspend(void)
{
	return 0;
}

static struct tegra_dc_mode dsi_a_768p_10_1_modes[] = {
	{
		.pclk = 71985000,
		.h_ref_to_sync = 4,
		.v_ref_to_sync = 1,
...
static struct tegra_dc_cmu dsi_a_768p_10_1_cmu = {
...
static int dsi_a_768p_10_1_bl_brightness_table[BOWSER_BL_TOTAL_LEVEL] =
	{15, 18, 26, 38, 56, 78, 97, 120, 142, 165, 190};

static int dsi_a_768p_10_1_bl_get_brightness(struct device *unused, int level)
{
	if (level > BOWSER_BL_TOTAL_LEVEL || level <= 0) {
		pr_info("Error: Brightness level is out of the range.(level=%d)\n", level);
		return -1;
	} else {
		return dsi_a_768p_10_1_bl_brightness_table[level -1];
	}
}

static int dsi_a_768p_10_1_bl_notify(struct device *unused, int brightness)
{
	...
	if (brightness > 255)
		...
		brightness = dsi_a_768p_10_1_bl_output_measured[brightness];
...
static int dsi_a_768p_10_1_check_fb(struct device *dev, struct fb_info *info)
{
	return info->device == &disp_device->dev;
}

static struct platform_pwm_backlight_data dsi_a_768p_10_1_bl_data = {
	.pwm_id		= 1,
	.max_brightness	= 255,
	.dft_brightness	= 142,	/* set to dsi_a_768p_10_1_bl_brightness_table[8] */
	.pwm_period_ns	= 1000000,
	.pwm_gpio = DSI_PANEL_BL_PWM,
	.notify		= dsi_a_768p_10_1_bl_notify,
	/* Only toggle backlight on fb blank notifications for disp1 */
	.check_fb	= dsi_a_768p_10_1_check_fb,
	.get_brightness = dsi_a_768p_10_1_bl_get_brightness,
};

static struct platform_device __maybe_unused dsi_a_768p_10_1_bl_device = {
	.name	= "pwm-backlight",
	.id	= -1,
	.dev	= {
		.platform_data = &dsi_a_768p_10_1_bl_data,
	},
};

static struct platform_device __maybe_unused *dsi_a_768p_10_1_bl_devices[] __initdata = {
	&tegra_pwfm1_device,
	&dsi_a_768p_10_1_bl_device,
};

static int __init dsi_a_768p_10_1_register_bl_dev(void)
{
	int err = 0;

	err = platform_add_devices(dsi_a_768p_10_1_bl_devices,
				ARRAY_SIZE(dsi_a_768p_10_1_bl_devices));
	if (err) {
...
static void dsi_a_768p_10_1_set_disp_device(struct platform_device *dalmore_display_device)
{
	disp_device = dalmore_display_device;
}

static void dsi_a_768p_10_1_resources_init(struct resource *resources, int n_resources)
{
	int i;
...
static void dsi_a_768p_10_1_dc_out_init(struct tegra_dc_out *dc)
{
	dc->dsi = &dsi_a_768p_10_1_pdata;
	dc->parent_clk = "pll_d_out0";
	dc->modes = dsi_a_768p_10_1_modes;
	dc->n_modes = ARRAY_SIZE(dsi_a_768p_10_1_modes);
	dc->enable = dsi_a_768p_10_1_enable;
	dc->postpoweron = dsi_a_768p_10_1_postpoweron;
	dc->disable = dsi_a_768p_10_1_disable;
	dc->prepoweroff = dsi_a_768p_10_1_prepoweroff;
	dc->postsuspend	= dsi_a_768p_10_1_postsuspend;
	dc->width = 216;
	dc->height = 135;
	dc->flags = DC_CTRL_MODE;
	dc->depth = 24;
	dc->h_padding = 2; // 1366+2 = 1368 For Toshiba TC358767
}

static void dsi_a_768p_10_1_fb_data_init(struct tegra_fb_data *fb)
{
	fb->xres = dsi_a_768p_10_1_modes[0].h_active;
	fb->yres = dsi_a_768p_10_1_modes[0].v_active;
}

static void dsi_a_768p_10_1_sd_settings_init(struct tegra_dc_sd_settings *settings)
{
	settings->bl_device_name = "pwm-backlight";
}

static void dsi_a_768p_10_1_cmu_init(struct tegra_dc_platform_data *pdata)
{
	pdata->cmu = &dsi_a_768p_10_1_cmu;
}
...
static int __init dsi_a_768p_10_1_i2c_bridge_register(void)
{
	int err = 0;
...
struct tegra_panel __initdata dsi_a_768p_10_1 = {
	.init_sd_settings = dsi_a_768p_10_1_sd_settings_init,
	.init_dc_out = dsi_a_768p_10_1_dc_out_init,
	.init_fb_data = dsi_a_768p_10_1_fb_data_init,
	.init_resources = dsi_a_768p_10_1_resources_init,
	.register_bl_dev = dsi_a_768p_10_1_register_bl_dev,
	.register_i2c_bridge = dsi_a_768p_10_1_i2c_bridge_register,
	.init_cmu_data = dsi_a_768p_10_1_cmu_init,
	.set_disp_device = dsi_a_768p_10_1_set_disp_device,
};

EXPORT_SYMBOL(dsi_a_768p_10_1);
