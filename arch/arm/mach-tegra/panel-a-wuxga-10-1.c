...
//#define DEBUG_FUNCTION	printk(KERN_WARNING "##################### %s [%d]\n", __FUNCTION__, __LINE__);
...
static struct regulator *vdd_lvds;

static tegra_dc_bl_output dsi_a_wuxga_10_1_bl_output_measured = {
		0,1,3,4,5,6,8,9,
		...
		240,242,243,244,245,246,247,249,
		250,251,252,252,253,254,254,255
};

static struct tegra_dsi_cmd dsi_a_wuxga_10_1_init_cmd[] = {
	/* no init command required */
};

static struct tegra_dsi_out dsi_a_wuxga_10_1_pdata = {
	...
	.dsi_init_cmd = dsi_a_wuxga_10_1_init_cmd,
	.n_init_cmd = ARRAY_SIZE(dsi_a_wuxga_10_1_init_cmd),
	.dsi2lvds_bridge_enable = true,
};

static int maya_dsi_regulator_get(struct device *dev)
{
	int err = 0;
DEBUG_FUNCTION;
	if (reg_requested)
...
static int maya_dsi_gpio_get(void)
{
DEBUG_FUNCTION;
...
static int dsi_a_wuxga_10_1_enable(struct device *dev)
{
	int err = 0;
	struct tegra_dc_out *pdata = ((struct tegra_dc_platform_data*)(dev->platform_data))->default_out;
DEBUG_FUNCTION;
	...
	if (vdd_lvds) {
		err = regulator_enable(vdd_lvds);
		if (err < 0) {
			pr_err("vdd_lvds regulator enable failed\n");
			goto fail;
		}
	}
...
static int dsi_a_wuxga_10_1_disable(void)
{
DEBUG_FUNCTION;
...
static int dsi_a_wuxga_10_1_postsuspend(void)
{
DEBUG_FUNCTION;
	return 0;
}

static struct tegra_dc_mode dsi_a_wuxga_10_1_modes[] = {
	{
		.pclk = 152520000,
		.h_ref_to_sync = 4,
		.v_ref_to_sync = 1,
...
static struct tegra_dc_cmu dsi_a_wuxga_10_1_cmu = {
...
static int dsi_a_wuxga_10_1_bl_notify(struct device *unused, int brightness)
{
...
DEBUG_FUNCTION;
	...
	if (brightness > 255)
		...
		brightness = dsi_a_wuxga_10_1_bl_output_measured[brightness];

	return brightness;
}

static int dsi_a_wuxga_10_1_check_fb(struct device *dev, struct fb_info *info)
{
DEBUG_FUNCTION;
	return info->device == &disp_device->dev;
}

static struct platform_pwm_backlight_data dsi_a_wuxga_10_1_bl_data = {
	.pwm_id		= 1,
	.max_brightness	= 255,
	.dft_brightness	= 255,
	.pwm_period_ns	= 1000000,
	.pwm_gpio = DSI_PANEL_BL_PWM,
	.notify		= dsi_a_wuxga_10_1_bl_notify,
	/* Only toggle backlight on fb blank notifications for disp1 */
	.check_fb	= dsi_a_wuxga_10_1_check_fb,
};

static struct platform_device __maybe_unused dsi_a_wuxga_10_1_bl_device = {
	.name	= "pwm-backlight",
	.id	= -1,
	.dev	= {
		.platform_data = &dsi_a_wuxga_10_1_bl_data,
	},
};

static struct platform_device __maybe_unused *dsi_a_wuxga_10_1_bl_devices[] __initdata = {
	&tegra_pwfm1_device,
	&dsi_a_wuxga_10_1_bl_device,
};

static int  __init dsi_a_wuxga_10_1_register_bl_dev(void)
{
	int err = 0;
DEBUG_FUNCTION;
	err = platform_add_devices(dsi_a_wuxga_10_1_bl_devices,
				ARRAY_SIZE(dsi_a_wuxga_10_1_bl_devices));
	if (err) {
...
static void dsi_a_wuxga_10_1_set_disp_device(
	struct platform_device *maya_display_device)
{
DEBUG_FUNCTION;
	disp_device = maya_display_device;
}

static void dsi_a_wuxga_10_1_resources_init(struct resource *
...
static void dsi_a_wuxga_10_1_dc_out_init(struct tegra_dc_out *dc)
{
DEBUG_FUNCTION;
	dc->dsi = &dsi_a_wuxga_10_1_pdata;
	dc->parent_clk = "pll_d_out0";
	dc->modes = dsi_a_wuxga_10_1_modes;
	dc->n_modes = ARRAY_SIZE(dsi_a_wuxga_10_1_modes);
	dc->enable = dsi_a_wuxga_10_1_enable;
	dc->disable = dsi_a_wuxga_10_1_disable;
	dc->postsuspend	= dsi_a_wuxga_10_1_postsuspend,
	dc->width = 217;
	dc->height = 135;
	dc->flags = DC_CTRL_MODE;
}

static void dsi_a_wuxga_10_1_fb_data_init(struct tegra_fb_data *fb)
{
DEBUG_FUNCTION;
	fb->xres = dsi_a_wuxga_10_1_modes[0].h_active;
	fb->yres = dsi_a_wuxga_10_1_modes[0].v_active;
}

static void
dsi_a_wuxga_10_1_sd_settings_init(struct tegra_dc_sd_settings *settings)
{
DEBUG_FUNCTION;
	settings->bl_device_name = "pwm-backlight";
}

static void dsi_a_wuxga_10_1_cmu_init(struct tegra_dc_platform_data *pdata)
{
DEBUG_FUNCTION;
	pdata->cmu = &dsi_a_wuxga_10_1_cmu;
}

struct tegra_panel __initdata dsi_a_wuxga_10_1 = {
	.init_sd_settings = dsi_a_wuxga_10_1_sd_settings_init,
	.init_dc_out = dsi_a_wuxga_10_1_dc_out_init,
	.init_fb_data = dsi_a_wuxga_10_1_fb_data_init,
	.init_resources = dsi_a_wuxga_10_1_resources_init,
	.register_bl_dev = dsi_a_wuxga_10_1_register_bl_dev,
	.init_cmu_data = dsi_a_wuxga_10_1_cmu_init,
	.set_disp_device = dsi_a_wuxga_10_1_set_disp_device,
};
EXPORT_SYMBOL(dsi_a_wuxga_10_1);
