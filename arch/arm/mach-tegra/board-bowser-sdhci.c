/*
 * arch/arm/mach-tegra/board-bowser-sdhci.c
 *
...
#define BOWSER_WLAN_PWR	TEGRA_GPIO_PCC5
#define BOWSER_WLAN_RST	TEGRA_GPIO_PX7
#define BOWSER_WLAN_WOW	TEGRA_GPIO_PU5
#define BOWSER_SD_CD		TEGRA_GPIO_PV2

static void (*wifi_status_cb)(int card_present, void *dev_id);
static void *wifi_status_cb_devid;
static int bowser_wifi_status_register(void (*callback)(int , void *), void *);
...
static struct platform_device bowser_wifi_device = {
	.name		= "bcm4329_wlan",
	.id		= 1,
	.num_resources	= 1,
...
static struct regulator *bowser_vdd_com_3v3;
static struct regulator *bowser_vddio_com_1v8;
#define BOWSER_VDD_WIFI_3V3 "vdd_wifi_3v3"
#define BOWSER_VDD_WIFI_1V8 "vddio_wifi_1v8"

static int bowser_wifi_regulator_enable(void)
{
	int ret = 0;

	/* Enable COM's vddio_com_1v8 regulator*/
	if (IS_ERR_OR_NULL(bowser_vddio_com_1v8)) {
		bowser_vddio_com_1v8 = regulator_get(&bowser_wifi_device.dev,
			BOWSER_VDD_WIFI_1V8);
		if (IS_ERR_OR_NULL(bowser_vddio_com_1v8)) {
			pr_err("Couldn't get regulator "
				BOWSER_VDD_WIFI_1V8 "\n");
			regulator_disable(bowser_vdd_com_3v3);

			regulator_put(bowser_vdd_com_3v3);
			bowser_vdd_com_3v3 = NULL;
			return PTR_ERR(bowser_vddio_com_1v8);
		}

		ret = regulator_enable(bowser_vddio_com_1v8);
		if (ret < 0) {
			pr_err("Couldn't enable regulator "
				BOWSER_VDD_WIFI_1V8 "\n");
			regulator_put(bowser_vddio_com_1v8);
			bowser_vddio_com_1v8 = NULL;
        ...
        if (IS_ERR_OR_NULL(bowser_vdd_com_3v3)) {
                bowser_vdd_com_3v3 = regulator_get(&bowser_wifi_device.dev,
                                                        BOWSER_VDD_WIFI_3V3);
                if (IS_ERR_OR_NULL(bowser_vdd_com_3v3)) {
                        pr_err("Couldn't get regulator "
                                BOWSER_VDD_WIFI_3V3 "\n");
                        return PTR_ERR(bowser_vdd_com_3v3);
                }

                ret = regulator_enable(bowser_vdd_com_3v3);
                if (ret < 0) {
                        pr_err("Couldn't enable regulator "
                                BOWSER_VDD_WIFI_3V3 "\n");
                        regulator_put(bowser_vdd_com_3v3);
...
static void bowser_wifi_regulator_disable(void)
{
	...
	if (!IS_ERR_OR_NULL(bowser_vddio_com_1v8)) {
		regulator_disable(bowser_vddio_com_1v8);
		regulator_put(bowser_vddio_com_1v8);
		bowser_vddio_com_1v8 = NULL;
	}
}
...
static int bowser_wifi_power(int on)
{
	...
	if (on == 1) {
		ret = bowser_wifi_regulator_enable();
		if (ret < 0) {
	...
	if (on) {
               gpio_set_value(BOWSER_WLAN_PWR, on);
               mdelay(100);
        }
	gpio_set_value(BOWSER_WLAN_RST, on);
	mdelay(200);

	/* Disable COM's regulators on wi-fi poer off*/
	if (on != 1) {
		pr_debug("Disabling COM regulators\n");
		bowser_wifi_regulator_disable();
	}
...
static int __init bowser_wifi_init(void)
{
	int rc;

	rc = gpio_request(BOWSER_WLAN_PWR, "wlan_power");
	if (rc)
		pr_err("WLAN_PWR gpio request failed:%d\n", rc);
	rc = gpio_request(BOWSER_WLAN_RST, "wlan_rst");
	if (rc)
		pr_err("WLAN_RST gpio request failed:%d\n", rc);
	rc = gpio_request(BOWSER_WLAN_WOW, "bcmsdh_sdmmc");
	if (rc)
		pr_err("WLAN_WOW gpio request failed:%d\n", rc);

	rc = gpio_direction_output(BOWSER_WLAN_PWR, 0);
	if (rc)
		pr_err("WLAN_PWR gpio direction configuration failed:%d\n", rc);
	gpio_direction_output(BOWSER_WLAN_RST, 0);
	if (rc)
		pr_err("WLAN_RST gpio direction configuration failed:%d\n", rc);
	rc = gpio_direction_input(BOWSER_WLAN_WOW);
	if (rc)
		pr_err("WLAN_WOW gpio direction configuration failed:%d\n", rc);

	wifi_resource[0].start = wifi_resource[0].end =
		gpio_to_irq(BOWSER_WLAN_WOW);

	platform_device_register(&bowser_wifi_device);
	return 0;
}

#ifdef CONFIG_TEGRA_PREPOWER_WIFI
static int __init bowser_wifi_prepower(void)
{
...
subsys_initcall_sync(bowser_wifi_prepower);
#endif

int __init bowser_sdhci_init(void)
{
	int nominal_core_mv;
