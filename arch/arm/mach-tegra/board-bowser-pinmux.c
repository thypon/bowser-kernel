/*
 * arch/arm/mach-tegra/board-bowser-pinmux.c
 *
...
static __initdata struct tegra_drive_pingroup_config bowser_drive_pinmux[] = {
...
static __initdata struct tegra_pingroup_config bowser_pinmux_set_nontristate[] = {
	...
	DEFAULT_PINMUX(GMI_AD3,         GMI,    PULL_UP,      NORMAL,    OUTPUT),
	DEFAULT_PINMUX(GMI_AD4,         GMI,    PULL_UP,      NORMAL,    OUTPUT),
...
#include "board-bowser-pinmux-t11x.h"
...
static void __init bowser_gpio_init_configure(void)
{
	int len;
	int i;
	struct gpio_init_pin_info *pins_info;

	len = ARRAY_SIZE(init_gpio_mode_bowser_common);
	pins_info = init_gpio_mode_bowser_common;
...
int __init bowser_pinmux_init(void)
{
	tegra_pinmux_config_table(bowser_pinmux_set_nontristate,
					ARRAY_SIZE(bowser_pinmux_set_nontristate));
	bowser_gpio_init_configure();

	tegra_pinmux_config_table(bowser_pinmux_common, ARRAY_SIZE(bowser_pinmux_common));
	tegra_drive_pinmux_config_table(bowser_drive_pinmux,
					ARRAY_SIZE(bowser_drive_pinmux));
