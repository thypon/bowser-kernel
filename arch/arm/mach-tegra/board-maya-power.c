/*
 * arch/arm/mach-tegra/board-maya-power.c
 *
...
static struct regulator_init_data *maya_e1611_reg_data[PALMAS_NUM_REGS] = {
...
static struct palmas_reg_init *maya_e1611_reg_init[PALMAS_NUM_REGS] = {
...
#define MAYA_POWER_CONFIG_2			\
	ADD_FIXED_REG(avdd_hdmi_pll),

/* Gpio switch regulator platform data for Maya E1611 */
static struct platform_device *fixed_reg_devs_e1611_a00[] = {
	MAYA_COMMON_FIXED_REG
	E1611_FIXED_REG
};

/* Gpio switch regulator platform data for Maya E1612 */
static struct platform_device *fixed_reg_devs_e1612_a00[] = {
	MAYA_COMMON_FIXED_REG
	E1612_FIXED_REG
};

static struct platform_device *fixed_reg_devs_maya_config2[] = {
	MAYA_POWER_CONFIG_2
};

static void set_maya_power_fab05(void)
{
	maya_e1611_reg_data[PALMAS_REG_LDOLN] =
				PALMAS_REG_PDATA(ldoln_fab05);
	maya_e1611_reg_init[PALMAS_REG_LDOLN] =
				PALMAS_REG_INIT_DATA(ldoln_fab05);
	maya_e1611_reg_data[PALMAS_REG_LDOUSB] =
				PALMAS_REG_PDATA(ldousb_fab05);
	maya_e1611_reg_init[PALMAS_REG_LDOUSB] =
				PALMAS_REG_INIT_DATA(ldousb_fab05);
	return;
}

static void set_maya_power_config2(void)
{
	maya_e1611_reg_data[PALMAS_REG_SMPS8] =
				PALMAS_REG_PDATA(smps8_config2);
	maya_e1611_reg_init[PALMAS_REG_SMPS8] =
				PALMAS_REG_INIT_DATA(smps8_config2);
	maya_e1611_reg_data[PALMAS_REG_LDO1] =
				PALMAS_REG_PDATA(ldo1_config2);
	maya_e1611_reg_init[PALMAS_REG_LDO1] =
				PALMAS_REG_INIT_DATA(ldo1_config2);
	maya_e1611_reg_data[PALMAS_REG_LDO2] =
				PALMAS_REG_PDATA(ldo2_config2);
	maya_e1611_reg_init[PALMAS_REG_LDO2] =
				PALMAS_REG_INIT_DATA(ldo2_config2);
	maya_e1611_reg_data[PALMAS_REG_LDO4] =
				PALMAS_REG_PDATA(ldo4_config2);
	maya_e1611_reg_init[PALMAS_REG_LDO4] =
				PALMAS_REG_INIT_DATA(ldo4_config2);
	return;
}

int __init maya_palmas_regulator_init(void)
{
	...
	if (board_info.fab == BOARD_FAB_A05) {
		set_maya_power_config2();
		set_maya_power_fab05();
	} else if (power_config & POWER_CONFIG2) {
		set_maya_power_config2();
	}

	for (i = 0; i < PALMAS_NUM_REGS ; i++) {
		pmic_platform.reg_data[i] = maya_e1611_reg_data[i];
		pmic_platform.reg_init[i] = maya_e1611_reg_init[i];
	}
...
static struct resource maya_pda_resources[] = {
	[0] = {
		.name	= "ac",
	},
};

static struct pda_power_pdata maya_pda_data = {
	.is_ac_online	= ac_online,
};

static struct platform_device maya_pda_power_device = {
	.name		= "pda-power",
	.id		= -1,
	.resource	= maya_pda_resources,
	.num_resources	= ARRAY_SIZE(maya_pda_resources),
	.dev	= {
		.platform_data	= &maya_pda_data,
	},
};

static struct tegra_suspend_platform_data maya_suspend_data = {
	.cpu_timer	= 500,
	.cpu_off_timer	= 300,
...
static struct tegra_cl_dvfs_cfg_param maya_cl_dvfs_param = {
	.sample_rate = 12500,
...
#ifdef CONFIG_ARCH_TEGRA_HAS_CL_DVFS
static struct tegra_cl_dvfs_platform_data maya_cl_dvfs_data = {
	...
	.cfg_param = &maya_cl_dvfs_param,
};

static int __init maya_cl_dvfs_init(void)
{
	fill_reg_map();
	if (tegra_revision < TEGRA_REVISION_A02)
		maya_cl_dvfs_data.flags = TEGRA_CL_DVFS_FLAGS_I2C_WAIT_QUIET;
	tegra_cl_dvfs_device.dev.platform_data = &maya_cl_dvfs_data;
...
static int __init maya_max77663_regulator_init(void)
{
	struct board_info board_info;
	void __iomem *pmc = IO_ADDRESS(TEGRA_PMC_BASE);
	u32 pmc_ctrl;
...
static struct regulator_bulk_data maya_gps_regulator_supply[] = {
	[0] = {
		.supply	= "vdd_gps_3v3",
	},
	[1] = {
...
static struct regulator_userspace_consumer_data maya_gps_regulator_pdata = {
	.num_supplies	= ARRAY_SIZE(maya_gps_regulator_supply),
	.supplies	= maya_gps_regulator_supply,
};

static struct platform_device maya_gps_regulator_device = {
	.name	= "reg-userspace-consumer",
	.id	= 2,
	.dev	= {
			.platform_data = &maya_gps_regulator_pdata,
	},
};

static int __init maya_fixed_regulator_init(void)
{
	struct board_info board_info;
	u8 power_config;
#ifndef CONFIG_MACH_MAYA
	...
	if (board_info.fab == BOARD_FAB_A05 || power_config & POWER_CONFIG2)
		platform_add_devices(fixed_reg_devs_maya_config2,
				ARRAY_SIZE(fixed_reg_devs_maya_config2));
...
subsys_initcall_sync(maya_fixed_regulator_init);

static void maya_tps65090_init(void)
{
	int err;
...
int __init maya_regulator_init(void)
{
	struct board_info board_info;
	...
	if (board_info.board_id == BOARD_E1611 ||
		board_info.board_id == BOARD_P2454)
		maya_palmas_regulator_init();
	else
		maya_max77663_regulator_init();

	i2c_register_board_info(4, tps51632_boardinfo, 1);
	platform_device_register(&maya_pda_power_device);
	platform_device_register(&maya_gps_regulator_device);
	return 0;
}

int __init maya_suspend_init(void)
{
	tegra_init_suspend(&maya_suspend_data);
	/* Enable maya USB wake for VBUS/ID without using PMIC */
	tegra_set_usb_vbus_internal_wake(
		maya_suspend_data.usb_vbus_internal_wake);
	tegra_set_usb_id_internal_wake(
		maya_suspend_data.usb_id_internal_wake);
	return 0;
}

int __init maya_edp_init(void)
{
...
static struct thermal_zone_params maya_soctherm_therm_cpu_tzp = {
...
static struct soctherm_platform_data maya_soctherm_data = {
	.therm = {
		[THERM_CPU] = {
			.zone_enable = true,
			.passive_delay = 1000,
			...
			.tzp = &maya_soctherm_therm_cpu_tzp,
		},
		[THERM_GPU] = {
			.zone_enable = true,
			.passive_delay = 1000,
...
int __init maya_soctherm_init(void)
{
	struct board_info board_info;
	...
	if (!(board_info.board_id == BOARD_E1611 ||
		board_info.board_id == BOARD_P2454))
		maya_soctherm_data.tshut_pmu_trip_data = &tpdata_max77663;

	tegra_platform_edp_init(maya_soctherm_data.therm[THERM_CPU].trips,
			&maya_soctherm_data.therm[THERM_CPU].num_trips,
			6000); /* edp temperature margin */
	tegra_add_tj_trips(maya_soctherm_data.therm[THERM_CPU].trips,
			&maya_soctherm_data.therm[THERM_CPU].num_trips);
	tegra_add_vc_trips(maya_soctherm_data.therm[THERM_CPU].trips,
			&maya_soctherm_data.therm[THERM_CPU].num_trips);

	return tegra11_soctherm_init(&maya_soctherm_data);
}
