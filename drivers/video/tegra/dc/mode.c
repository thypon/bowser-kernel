...
static s64 calc_frametime_ns(const struct tegra_dc *dc,
	const struct tegra_dc_mode *m)
{
	long h_total, v_total;
	int padding = dc && dc->out ? dc->out->h_padding : 0;

	h_total = m->h_active + padding + m->h_front_porch + m->h_back_porch +
		m->h_sync_width;
...
int tegra_dc_calc_refresh(struct tegra_dc *dc, const struct tegra_dc_mode *m)
{
	...
	h_total = m->h_active + padding + m->h_front_porch +
		m->h_back_porch + m->h_sync_width;
	v_total = m->v_active + m->v_front_porch + m->v_back_porch +
		m->v_sync_width;
...
int tegra_dc_program_mode(struct tegra_dc *dc, struct tegra_dc_mode *mode)
{
	...
	tegra_dc_writel(dc, mode->h_back_porch | (mode->v_back_porch << 16),
			DC_DISP_BACK_PORCH);
	tegra_dc_writel(dc, tegra_dc_h_active_real(dc) | (mode->v_active << 16),
			DC_DISP_DISP_ACTIVE);
	...
	switch_set_state(&dc->modeset_switch,
			 (tegra_dc_h_active_real(dc) << 16) | mode->v_active);
#endif
...
int tegra_dc_set_mode(struct tegra_dc *dc, const struct tegra_dc_mode *mode)
{
	...
	if (dc->out->type == TEGRA_DC_OUT_RGB)
		panel_sync_rate = tegra_dc_calc_refresh(dc, mode);
	else if (dc->out->type == TEGRA_DC_OUT_DSI)
		panel_sync_rate = dc->out->dsi->rated_refresh_rate * 1000;

	print_mode(dc, mode, __func__);
	dc->frametime_ns = calc_frametime_ns(dc, mode);
...
int tegra_dc_to_fb_videomode(struct tegra_dc *dc, struct fb_videomode *fbmode,
	const struct tegra_dc_mode *mode)
{
	...
	fbmode->refresh = tegra_dc_calc_refresh(dc, mode) / 1000;
