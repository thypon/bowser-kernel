					...
					struct tegra_dc_dsi_data *dsi)
{
	...
	h_width_pixels = dc->mode.h_back_porch + dc->mode.h_front_porch +
			dc->mode.h_sync_width + tegra_dc_h_active_real(dc);

	if (dsi->info.ganged_type == TEGRA_DSI_GANGED_SYMMETRIC_EVEN_ODD) {
		temp = tegra_dc_h_active_real(dc) % dsi->info.n_data_lanes;
		if (temp) {
				...
				struct tegra_dc_dsi_data *dsi)
{
	...
	temp = tegra_dc_h_active_real(dc) * 1000;
	temp1 = tegra_dc_h_active_real(dc) + dc_modes->h_back_porch +
	...
	if (sol_delay > (480 * 4)) {
		...
		mipi_clk_adj_kHz = sol_delay +
			(tegra_dc_h_active_real(dc) * dsi->pixel_scaler_mul) /
...
static void tegra_dsi_setup_ganged_mode_pkt_length(struct tegra_dc *dc,
						struct tegra_dc_dsi_data *dsi)
{
	u32 hact_pkt_len_pix_orig = tegra_dc_h_active_real(dc);
...
#define HEADER_OVERHEAD 16
	...
	pix_per_line_orig = dc->mode.h_sync_width + dc->mode.h_back_porch +
			tegra_dc_h_active_real(dc) + dc->mode.h_front_porch;
						...
						struct tegra_dc_dsi_data *dsi)
{
	...
	hact_pkt_len = tegra_dc_h_active_real(dc) * dsi->pixel_scaler_mul /
							dsi->pixel_scaler_div;
						...
						struct tegra_dc_dsi_data *dsi)
{
	...
	if (dsi->info.ganged_type) {
		act_bytes = DIV_ROUND_UP(tegra_dc_h_active_real(dc), 2);
		act_bytes = (act_bytes) * dsi->pixel_scaler_mul /
				dsi->pixel_scaler_div + 1;
	} else {
		act_bytes = tegra_dc_h_active_real(dc) * dsi->pixel_scaler_mul /
				dsi->pixel_scaler_div + 1;
	}
				...
				struct tegra_dc_dsi_data *dsi)
{
	u32 low_width = 0;
	u32 high_width = 0;
	u32 h_active = tegra_dc_h_active_real(dc);
	u32 val = 0;
