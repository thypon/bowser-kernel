...
static u8 dsi2lvds_config_clk[][2] = {
	...
	DSI2LVDS_REG_VAL(0x0b, 0x28), /* configure PLL */
...
static u8 dsi2lvds_config_dsi[][2] = {
	...
	DSI2LVDS_REG_VAL(0x12, 0x5C), /* channel A clk range */
...
static u8 dsi2lvds_config_video[][2] = {
	DSI2LVDS_REG_VAL(0x20, 0x80), /* horizontal pixels on dsi channel A */
	DSI2LVDS_REG_VAL(0x21, 0x07),
	DSI2LVDS_REG_VAL(0x22, 0x80), /* horizontal pixels on dsi channel B */
	DSI2LVDS_REG_VAL(0x23, 0x07),
	DSI2LVDS_REG_VAL(0x24, 0xB0), /* vertical pixels on lvds channel A */
	DSI2LVDS_REG_VAL(0x25, 0x04),
	DSI2LVDS_REG_VAL(0x26, 0xB0), /* vertical pixels on lvds channel B */
	DSI2LVDS_REG_VAL(0x27, 0x04),
	DSI2LVDS_REG_VAL(0x28, 0x21), /* Pixel clk delay from dsi to */
	DSI2LVDS_REG_VAL(0x29, 0x00), /* lvds channel A */
	DSI2LVDS_REG_VAL(0x2a, 0x21), /* Pixel clk delay from dsi to */
	DSI2LVDS_REG_VAL(0x2b, 0x00), /* lvds channel B */
	DSI2LVDS_REG_VAL(0x2c, 0x0A), /* hsync width channel A */
	DSI2LVDS_REG_VAL(0x2d, 0x00),
	DSI2LVDS_REG_VAL(0x2e, 0x0A), /* hsync width channel B */
	DSI2LVDS_REG_VAL(0x2f, 0x00),
	DSI2LVDS_REG_VAL(0x30, 0x02), /* vsync width channel A */
	DSI2LVDS_REG_VAL(0x31, 0x00),
	DSI2LVDS_REG_VAL(0x32, 0x02), /* vsync width channel B */
	DSI2LVDS_REG_VAL(0x33, 0x00),
	DSI2LVDS_REG_VAL(0x34, 0x14), /* h back porch channel A */
	DSI2LVDS_REG_VAL(0x35, 0x14), /* h back porch channel B */
	DSI2LVDS_REG_VAL(0x36, 0x02), /* v back porch channel A */
	DSI2LVDS_REG_VAL(0x37, 0x02), /* v back porch channel B */
	DSI2LVDS_REG_VAL(0x38, 0x3C), /* h front porch channel A */
	DSI2LVDS_REG_VAL(0x39, 0x3C), /* h front porch channel B */
	DSI2LVDS_REG_VAL(0x3a, 0x08), /* v front porch channel A */
	DSI2LVDS_REG_VAL(0x3b, 0x08), /* v front porch channel B */
	//DSI2LVDS_REG_VAL(0x3c, 0x10), /* channel A/B test pattern */
};
...
static int tegra_dsi2lvds_init(struct tegra_dc_dsi_data *dsi)
{
	...
	err = gpio_request(DSI_PANEL_RST_GPIO, "panel backlight");
	if (err < 0)
		printk(KERN_ERR "gpio_request DSI_PANEL_RST_GPIO failed\n");

	avdd_lcd_3v3 = regulator_get(&dsi->dc->ndev->dev, "vdd_lvds");
	if (IS_ERR_OR_NULL(avdd_lcd_3v3)) {
		printk("vdd_3v3_lvds regulator get failed\n");
...
static u8 dsi2lvds_i2c_read_byte(struct tegra_dc_dsi2lvds_data *dsi2lvds, u8 addr)
{
	u8 data;
	...
	if(data < 0)
	{
		printk(KERN_WARNING "error at %s %d\n", __FUNCTION__, __LINE__);
	}
	return data;
}
...
static void tegra_dsi2lvds_enable(struct tegra_dc_dsi_data *dsi)
{
	...
	err = dsi2lvds_i2c_read_byte(dsi2lvds, 0x0a);
	if(err & 0x80)
	{
	...
	err = dsi2lvds_i2c_transfer(dsi2lvds, dsi2lvds_soft_reset, ARRAY_SIZE(dsi2lvds_soft_reset), I2C_WRITE);
	if(err < 0)
	{
		printk(KERN_WARNING "Error when soft reset\n");
	}
