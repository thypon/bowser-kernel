...
int inv_hw_self_test(struct inv_mpu_iio_s *st)
{
	...
	if (st->chip_config.is_asleep      ||
		...
		if (result)
		{
			printk("Failed to run Self Test: Setup conditions fail\n");
			return result;
		}
	}
	mutex_lock(&st->mutex);    
	...
	if (result) {
		printk("Failed to run Self Test 2: Unable to get biases\n");
		goto test_fail;
	}
	...
	if (result) {
		printk("Failed to run Self Test 3: Unable to get ST_biases\n");
		goto test_fail;
	}
...
test_fail:
        inv_recover_setting(st);
        return (compass_result<<DEF_ST_COMPASS_RESULT_SHIFT) | (accel_result<<DEF_ST_ACCEL_RESULT_SHIFT) | gyro_result;
