...
static DEVICE_ATTR(cmd, S_IWUGO, NULL, user_cmd);
...
static int io373b_acpi_probe(struct platform_device *pdev)
{
	...
	if (!info) {
		dev_err(&pdev->dev, "io373b_subdev_board_info.acpi not specified.\n");
		return -EINVAL;
	}

	num_bat = info->acpi.num_bat;
	if (num_bat <= 0 || num_bat > 2) {
		dev_err(&pdev->dev, "Invalid io373b_subdev_board_info.acpi.num_bat (%d)\n", num_bat);
		return -EINVAL;
	}
