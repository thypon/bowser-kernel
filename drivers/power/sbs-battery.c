...
#define TEGRA_GPIO_PH4	60	//EC_GMI_AD12
#define EC_Charging_Alert	0
#define EC_Discharging_Alert	1
...
/* Add for additional battery information */
enum bat_info_name {
	BAT_DESIGN_CAPACITY,
	...
	BAT_CELL_VOLTAGE3,
	BAT_SERIAL_NUMBER,
	BAT_MANUFACTURE_NAME,
	BAT_BATTERY_CT_NUMBER,
	BAT_DEVICE_NAME,
	BAT_DEVICE_CHEMISTRY,
	BAT_MANUFACTURE_DATE,
...
static const struct bat_data {
	enum bat_info_name name;
	u8 addr;
	int len;
} bat_info[] ={
	{BAT_DESIGN_CAPACITY,		0x18, 2},
	{BAT_FULL_CHARGE_CAPACITY,	0x10, 2},
	{BAT_REMAINING_CAPACITY,	0x0F, 2},
	{BAT_MAX_ERROR,			0x0C, 2},
	{BAT_CYCLE_COUNT,		0x17, 2},
	{BAT_TEMPERATURE,		0x08, 2},
	{BAT_VOLTAGE,			0x09, 2},
	{BAT_CURRENT,			0x0A, 2},
	{BAT_DESIGN_VOLTAGE,		0x19, 2},
	{BAT_BATTERY_STATUS,		0x16, 2},
	{BAT_CELL_VOLTAGE1,		0x3F, 2},
	{BAT_CELL_VOLTAGE2,		0x3E, 2},
	{BAT_CELL_VOLTAGE3,		0x3D, 2},
	{BAT_SERIAL_NUMBER,		0x1C, 2},
	{BAT_MANUFACTURE_NAME,		0x20, 18+1},
	{BAT_BATTERY_CT_NUMBER,		0x70, 16+1},
	{BAT_DEVICE_NAME,		0x21, 7+1},
	{BAT_DEVICE_CHEMISTRY,		0x22, 4+1},
	{BAT_MANUFACTURE_DATE,		0x1B, 2},
	{BAT_CHARGING_CURRENT,		0x14, 2},
	{BAT_CHARGING_VOLTAGE,		0x15, 2},
	{BAT_AVERAGE_CURRENT,		0x0B, 2},
	{BAT_RUN_TIME_TO_EMPTY,		0x11, 2},
	{BAT_AVERAGE_TIME_TO_EMPTY,	0x12, 2},
	{BAT_AVERAGE_TIME_TO_FULL,	0x13, 2},
};

static ssize_t user_data(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct sbs_info *chip = dev_get_drvdata(dev);
	int ret, i, j, n_char = 0;
	unsigned char data[256];

	for (i = 0; i < ARRAY_SIZE(bat_info_string); i++) {
		// Add name
		n_char += sprintf(buf + n_char, "%s=", bat_info_string[i]);

		// Add value
		if (bat_info[i].len == 2) {
			// int value
			ret = i2c_smbus_read_word_data(chip->client, bat_info[i].addr);
			if (ret < 0)
				return ret;

			switch (i) {
				case BAT_CURRENT:
				case BAT_CHARGING_CURRENT:
				case BAT_AVERAGE_CURRENT:
					n_char += sprintf(buf + n_char, "%d", (int16_t)ret);
					break;

				case BAT_TEMPERATURE:
					n_char += sprintf(buf + n_char, "%d", ret-2731);
					break;

				case BAT_SERIAL_NUMBER:
					n_char += sprintf(buf + n_char, "%04x", ret);
					break;

				default:
					n_char += sprintf(buf + n_char, "%d", ret);
					break;
			}
		} else {
			// string value
			ret = i2c_smbus_read_i2c_block_data(chip->client, bat_info[i].addr, bat_info[i].len, data);
			if (ret < 0)
				return ret;

			for (j = 1; j < bat_info[i].len; j++) {
				if ((data[j] < 0x20) || (data[j] > 0x7F))
					buf[n_char + j - 1] = ' ';
				else
					buf[n_char + j - 1] = data[j];
			}
			n_char += bat_info[i].len - 1;
		}
		// Add LF
...
static int sbs_create_dev_attrs(struct sbs_info *chip)
{
	int err;

	if ((err = device_create_file(&chip->client->dev, &dev_attr_data)))
	{
		dev_err(&chip->client->dev, "Failed to create attr data - %d\n", err);
		device_remove_file(&chip->client->dev, &dev_attr_data);
		return err;
	}
...
static void sbs_remove_dev_attrs(struct sbs_info *chip)
{
	device_remove_file(&chip->client->dev, &dev_attr_data);
}
...
Discharging : High
*/
static void ec_charging_alert_gpio(int charging)
{
	int gpio_value;

	gpio_value = gpio_get_value(TEGRA_GPIO_PH4);

	if(gpio_value != charging)
		gpio_set_value(TEGRA_GPIO_PH4, charging);
}
	...
	int *val)
{
	...
	if (ret >= sbs_data[reg_offset].min_value &&
		...
		*val = ret;
		...
		{
			ec_charging_alert_gpio(EC_Discharging_Alert);
			*val = POWER_SUPPLY_STATUS_FULL;
		}
		else if (ret & BATTERY_FULL_DISCHARGED)
		{
			/*if over discharge,need double check*/
			ret = sbs_read_word_data(client,0x0A);
			ret = (s16)ret;
			if (ret > 1)
			{
				ec_charging_alert_gpio(EC_Charging_Alert);
	 ...
	 enum power_supply_property psp, int *val)
{
	s32 ret;
	int reg_offset;
	enum sbs_battery_mode mode = BATTERY_MODE_AMPS;
...
static int init_EC_alert_gpio()
{
	int rc;

	rc = gpio_request(TEGRA_GPIO_PH4, "EC_CHARGE_ALERT");
	if (rc)
		pr_err("TEGRA_GPIO_PH4 gpio request failed:%d\n", rc);

	gpio_direction_output(TEGRA_GPIO_PH4, 0);
	if (rc)
		pr_err("TEGRA_GPIO_PH4 gpio direction configuration failed:%d\n", rc);
	...
	union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		// If status is FULL, set capacity as 100
		if (chip->battery_status == POWER_SUPPLY_STATUS_FULL)
			val->intval = 100;
		else {
			// Except FULL status, report real capacity
	...
	case POWER_SUPPLY_PROP_TEMP_AMBIENT:
		ret = sbs_read_word_data(client,0x16);
		if (ret < 0) //I2C Non-Ack
			val->intval = 0;
		else
			val->intval = 1;
		break;
...
void sbs_update(void)
{
	...
	if (tchip != NULL) {
		...
		if (ret & BATTERY_FULL_CHARGED) {
			...
			/*if over discharge,need double check*/
			ret = sbs_read_word_data(tchip->client,0x0A);
			ret = (s16)ret;
			if (ret > 1)
			{
				ec_charging_alert_gpio(EC_Charging_Alert);
...
battery_detect:
	...
	/* Send event every 2 seconds for Battery Widget to update
	*  base battery info even if slate battery status is the same.
	*/
...
int sbs_battery_status(void)
{	
	if(tchip != NULL &&  battery_detect)
	{
		if (tchip->battery_status == POWER_SUPPLY_STATUS_FULL)
			return 1;
		else 
			return 0;
	}
...
EXPORT_SYMBOL_GPL(sbs_battery_status);
