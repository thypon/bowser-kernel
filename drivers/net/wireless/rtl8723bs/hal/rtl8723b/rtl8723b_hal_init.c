...
s32 rtl8723b_FirmwareDownload(PADAPTER padapter, BOOLEAN  bUsedWoWLANFw)
{
	...
	{
		rtStatus = _FAIL;
		DBG_8192C("%s: Firmware size(%d) exceed %d. Check it!\n", __FUNCTION__, pFirmware->ulFwLength, FW_8723B_SIZE);
		goto Exit;
	}
...
void rtl8723b_InitBeaconParameters(PADAPTER padapter)
{
	...
	val16 |= EN_BCN_FUNCTION; // enable prot0 beacon function for PSTDMA
#endif
...
#if 1 // Modify antenna number setting for BT-Coex
	// Ant_x1 for 1 Antenna and Ant_x2 for 2
