...
void rtw_tdls_process_ht_cap(_adapter *adapter, struct sta_info *ptdls_sta, u8 *data, u8 Length)
{
	...
	if(ptdls_sta->htpriv.ht_option)
	{
		...
		if(ptdls_sta->ht_cap.cap_info & cpu_to_le16(IEEE80211_HT_CAP_SGI_20))
		{
			ptdls_sta->sgi_20m = _TRUE;
		}
		//check if sta support s Short GI 40M
		if(ptdls_sta->ht_cap.cap_info & cpu_to_le16(IEEE80211_HT_CAP_SGI_40))
		{
			ptdls_sta->sgi_40m = _TRUE;
		}
