...
static ssize_t rmi_fn_01_build_id_show(struct device *dev,
					 struct device_attribute *attr,
					 char *buf)
{
	struct rmi_function *fn = to_rmi_function(dev);
...
static struct device_attribute dev_attr_build_id =
	__ATTR(build_id, RMI_RO_ATTR,
		rmi_fn_01_build_id_show, NULL);
...
static int rmi_f01_create_sysfs(struct rmi_function *fn)
{
	...
	if (data->properties.has_build_id_query) {
		retval = sysfs_create_file(&fn->dev.kobj,
					   &dev_attr_build_id.attr);
		dev_warn(&fn->dev, "Failed to creat sysfs file for fw build id.\n");
	}
