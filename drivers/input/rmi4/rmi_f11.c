...
static void rmi_f11_rel_pos_report(struct f11_2d_sensor *sensor, u8 n_finger)
{
	...
	rel.x = data->rel_pos[n_finger * 2];
	rel.y = data->rel_pos[n_finger * 2 + 1];

	rel.x = min(F11_REL_POS_MAX, max(F11_REL_POS_MIN, (int)rel.x));
	rel.y = min(F11_REL_POS_MAX, max(F11_REL_POS_MIN, (int)rel.y));
	...
	if (axis_align->flip_x)
		rel.x = min(F11_REL_POS_MAX, -rel.x);
	if (axis_align->flip_y)
		rel.y = min(F11_REL_POS_MAX, -rel.y);

	if (rel.x || rel.y) {
		rmi_input_report_rel(sensor->rmi_input, &rel);
		if (!pdata->unified_input_device)
			rmi_input_report_rel(sensor->rmi_mouse_input, &rel);
	}
	if (!pdata->unified_input_device)
		input_sync(sensor->rmi_mouse_input->input_dev);
}
				   ...
				   u8 finger_state, u8 n_finger)
{
	...
	abs.tool_type = get_tool_type(sensor, finger_state);
	abs.n_finger = n_finger;
	if (finger_state) {
		//abs.n_finger = n_finger;
		...
		abs.orient = abs.w_x > abs.w_y ? 1 : 0;
		...
		*/
		...
		abs.x =  max(axis_align->clip_X_low, (int)abs.x);
		abs.y =  max(axis_align->clip_Y_low, (int)abs.y);
		if (axis_align->clip_X_high)
			abs.x = min(axis_align->clip_X_high, (int)abs.x);
		if (axis_align->clip_Y_high)
			abs.y =  min(axis_align->clip_Y_high, (int)abs.y);
	}
	...
	if (IS_ENABLED(CONFIG_RMI4_F11_PEN) &&
		...
		abs.w_max = max(1, (int)abs.w_max);
		abs.w_min = max(1, (int)abs.w_min);
	}
	if (sensor->type_a) {
		input_report_abs(sensor->rmi_input->input_dev,
				ABS_MT_TRACKING_ID, n_finger);
		input_report_abs(sensor->rmi_input->input_dev, ABS_MT_TOOL_TYPE,
					get_tool_type(sensor, finger_state));
	} else if (!pdata->unified_input_device) {
		input_mt_slot(sensor->rmi_input->input_dev, n_finger);
		input_mt_report_slot_state(sensor->rmi_input->input_dev,
			get_tool_type(sensor, finger_state), finger_state);
	}
	if (finger_state) {
		rmi_input_report_abs(sensor->rmi_input, &abs);
#if 0
		...
		rmi_input_report_state(sensor->rmi_input, RMI_INPUT_NO_FINGER, &abs);
	}

	/* MT sync between fingers */
	if (sensor->type_a)
		input_mt_sync(sensor->rmi_input->input_dev);

	if (sensor->sensor_type == rmi_sensor_touchpad)
		input_mt_report_pointer_emulation(sensor->rmi_input->input_dev,
							true);
}
	...
	if (sensor->sens_query.has_gestures &&
		...
		for (i = 0; i < virtualbutton_map->buttons; i++) {
			...
			if (x >= virtualbutton.x &&
				...
				rmi_input_report_key(sensor->rmi_input,
					virtualbutton_map->map[i].code, 1);
				rmi_input_report_key(sensor->rmi_input,
					virtualbutton_map->map[i].code, 0);
				input_sync(sensor->rmi_input->input_dev);
				return 0;
			}
		}
	}
				   ...
				   struct f11_2d_sensor *sensor)
{
	...
	for (i = 0; i < sensor->nbr_fingers; i++) {
		u8 abs_base = i * RMI_F11_ABS_BYTES;
		struct f11_2d_data *data = &sensor->data;
		...
		if(finger_state)
		{
			...
			w_max = int_sqrt( w_x*w_x + w_y*w_y );
			if (sensor->suppress_highw > 0
					&& sensor->suppress_highw <= w_max)
			{
				dev_dbg(&sensor->fn->dev,
	...
	rmi_input_report_key(sensor->rmi_input, BTN_TOUCH, finger_pressed_count);
...
static void f11_set_abs_params(struct rmi_function *fn, int index)
{
	...
	struct input_dev *input = sensor->rmi_input->input_dev;
...
static int rmi_f11_initialize(struct rmi_function *fn)
{
	...
	for (i = 0; i < (f11->nr_sensors + 1); i++) {
		...
		if(sensor->sens_query.has_dribble)
		{
			f11->dev_controls.ctrl0_9[0] |= RMI_F11_DRIBBLE;
			rc = f11_write_control_regs(fn,NULL,&f11->dev_controls,control_base_addr);
			if (rc < 0) {
				dev_err(&fn->dev,
				"Failed to write F11 control params.\n");
				return rc;
			}
		}
		else
		{
			dev_err(&fn->dev,
				"No dribble packet feature, it may cause problems on touchpads.\n");
		}
...
static void register_virtual_buttons(struct rmi_function *fn,
	...
	sensor->rmi_input->input_dev->keycode = sensor->button_map;
	sensor->rmi_input->input_dev->keycodesize = sizeof(u8);
	sensor->rmi_input->input_dev->keycodemax = sensor->virtual_buttons.buttons;

	/* set bits for each button... */
	for (j = 0; j < sensor->virtual_buttons.buttons; j++) {
		sensor->button_map[j] =  sensor->virtual_buttons.map[j].code;
		set_bit(sensor->button_map[j],
			sensor->rmi_input->input_dev->keybit);
	}
}
...
static int rmi_f11_register_devices(struct rmi_function *fn)
{
	...
	for (i = 0; i < (f11->nr_sensors + 1); i++) {
		...
		sensors_itertd = i;
		if (pdata->unified_input_device) {
			sensor->rmi_input = driver_data->rmi_input;
			input_dev = sensor->rmi_input->input_dev;
			rmi_touchpad_create(sensor->rmi_input, sensor->nbr_fingers, sensor->max_x, sensor->max_y);
		} else {
			sensor->rmi_input = rmi_input_dev_alloc(&rmi_dev->dev,
						driver_data, "%s.abs%d/input0", f11);
			if (!sensor->rmi_input) {
				dev_err(&fn->dev,
		...
		if (!pdata->unified_input_device) {
			rc = rmi_input_dev_register(sensor->rmi_input);
			if (rc < 0) {
				rmi_input_dev_free(sensor->rmi_input);
				goto error_unregister;
			}
		}
			...
			&& sensor->sens_query.has_rel)
		{
			...
			sensor->rmi_mouse_input = rmi_input_dev_alloc(
						&rmi_dev->dev, driver_data,
						"%s.rel%d/input0", NULL);
			if (!sensor->rmi_mouse_input) {
				dev_err(&fn->dev, "%s: Error in setting mouse input device.\n",
					__func__);
				goto error_unregister;
			}
			input_dev_mouse = sensor->rmi_mouse_input->input_dev;
			...
			rc = rmi_input_dev_register(sensor->rmi_mouse_input);
			if (rc < 0) {
				rmi_input_dev_free(sensor->rmi_mouse_input);
				goto error_unregister;
			}
...
error_unregister:
	if (!pdata->unified_input_device) {
		for (; sensors_itertd > 0; sensors_itertd--) {
			rmi_input_dev_unregister(
				f11->sensors[sensors_itertd].rmi_mouse_input);
			rmi_input_dev_unregister(f11->sensors[i].rmi_input);
		}
	}
...
static void rmi_f11_free_devices(struct rmi_function *fn)
{
	...
	if (!pdata->unified_input_device) {
		for (i = 0; i < (f11->nr_sensors + 1); i++) {
			rmi_input_dev_unregister(f11->sensors[i].rmi_input);
			rmi_input_dev_unregister(f11->sensors[i].rmi_mouse_input);
		}
	}
}
