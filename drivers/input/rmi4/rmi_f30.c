		...
		&& gpiled < f30->control.reg_2->length; gpiled++)
	{
		...
		if (!(f30->control.reg_0->regs[gpiled].led_sel)) {
			if (f30->control.reg_2->regs[gpiled].dir != 0) {
				...
				rmi_input_report_key(f30->rmi_input,
					f30->gpioled_map[gpiled], gpiled_status);
			}
		}
	}

	input_sync(f30->rmi_input->input_dev);
...
static int rmi_f30_register_device(struct rmi_function *fn)
{
	...
	if (pdata->unified_input_device) {
		f30->rmi_input = driver_data->rmi_input;
		input_dev = f30->rmi_input->input_dev;
	} else {
		f30->rmi_input = rmi_input_dev_alloc(&rmi_dev->dev,
						driver_data, "%s/input0", f30);
		if (rc < 0) {
	...
	if (!pdata->unified_input_device) {
		rc = rmi_input_dev_register(f30->rmi_input);
		if (rc < 0) {
...
error_free_device:
	if (!pdata->unified_input_device)
		rmi_input_dev_free(f30->rmi_input);
...
error_uregister_exit:
	if (!pdata->unified_input_device)
		rmi_input_dev_unregister(f30->rmi_input);
