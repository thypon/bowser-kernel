...
static int bypass_read_bma250(struct inv_gyro_state_s *st, unsigned char *data)
{
	int result;
	result = set_3050_bypass(st, 1);
	if (result)
	...
	result = inv_secondary_read(BMA250_X_AXIS_LSB_REG, 6, data);
...
static struct inv_mpu_slave slave_bma250 = {
	...
	.bypass_read = bypass_read_bma250
};
