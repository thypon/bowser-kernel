...
static int inv_set_lpf(struct inv_gyro_state_s *st, int rate)
{
	...
	if (INV_MPU3050 == st->chip_type) {
		...
		result = inv_i2c_single_write(st, reg->lpf,
			data | (st->chip_config.fsr << GYRO_CONFIG_FSR_SHIFT));
		if (result)
			return result;
	} else
			  ...
			  const char *buf, size_t count)
{
	...
	if (INV_MPU3050 == st->chip_type)
		...
		result = inv_i2c_single_write(st, reg->accl_config, (fs << ACCL_CONFIG_FSR_SHIFT));
	if (result)
		return result;
...
{
	struct inv_gyro_state_s *st = dev_get_drvdata(dev);
	return sprintf(buf, "%d\n", (int)((1 << st->chip_config.fsr)*GYRO_DPS_SCALE));
}
			 ...
			 struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf, "%d\n", (int)(ACCL_G_SCALE << st->chip_config.accl_fs));
}
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	if (INV_MPU3050 == st->chip_type) {
		result = inv_self_test_mpu3050(st, bias);
	} else if (INV_MPU6500 == st->chip_type) {
		result = inv_hw_self_test_6500(st, bias);
	} else {
 ...
 */
ssize_t inv_get_accl_bias_show(struct device *dev,
				      struct device_attribute *attr, char *buf)
{
	int result;
	int bias[3];
	struct inv_gyro_state_s *st = dev_get_drvdata(dev);
    if(INV_MPU3050 == st->chip_type)
        result = inv_get_accl_bias_mpu3050(st, bias);
    else
...
/**
 * inv_reg_write_show() - last register write exit value for testing.
 */
static ssize_t inv_reg_write_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "1\n");
}
...
#ifdef CONFIG_INV_TESTING
static DEVICE_ATTR(reg_write, S_IRUGO | S_IWUSR,
		   inv_reg_write_show, inv_reg_write_store);
#endif
...
//check 3050/6050/6500
static int inv_check_chip_type_dynamic(struct inv_gyro_state_s *st,
		const struct i2c_device_id *id)
{
    ...
    inv_i2c_read(st, 0x00, 1, &temp);
    printk("0x00=0x%x\n", temp);
    inv_i2c_read(st, 0x01, 1, &temp);
    printk("0x01=0x%x\n", temp);
    inv_i2c_read(st, 0x3E, 1, &temp);
    printk("0x3E=0x%x\n", temp);
    inv_i2c_read(st, 0x6B, 1, &temp);
    printk("0x6B=0x%x\n", temp);
    inv_i2c_read(st, 0x75, 1, &temp);
    printk("0x75=0x%x\n", temp);

    result = inv_i2c_read(st, REG_WHOAMI, 1, &who_am_i);
	if (result)
		return result;
    who_am_i &= WHOAMI_MASK;
    printk("who am i[reg.0x%x] = 0x%x\n", REG_WHOAMI, who_am_i);
    if (who_am_i == MPU6500_ID) {
		st->chip_type = INV_MPU6500;
        printk("%s, INV_MPU6500\n", __func__);
    } else if (who_am_i == MPU3050_6050_ID) {
		st->chip_type = INV_MPU6050;
        printk("%s, INV_MPU6050\n", __func__);
    } else {
        result = inv_i2c_read(st, REG_WHOAMI_3050, 1, &who_am_i);
        if (result)
            return result;
        who_am_i &= WHOAMI_MASK;
        printk("who am i[reg.0x%x] = 0x%x\n", REG_WHOAMI_3050, who_am_i);
        if (who_am_i == MPU3050_6050_ID)
        {
		    st->chip_type = INV_MPU3050;
            printk("%s, INV_MPU3050\n", __func__);
        }
    }

    //we assume
    //MPU3050:              BMA250 on 2nd bus
    //MPU6050/6500:     nothing on 2nd bus
    if (st->chip_type == INV_MPU3050) {
        st->plat_data.sec_slave_type = SECONDARY_SLAVE_TYPE_ACCEL;
        st->plat_data.sec_slave_id = ACCEL_ID_BMA250;
    } else {
        st->plat_data.sec_slave_type = SECONDARY_SLAVE_TYPE_NONE;
        st->plat_data.sec_slave_id = ID_INVALID;
    }
			       ...
			       const struct i2c_device_id *id)
{
    ...
    if (SECONDARY_SLAVE_TYPE_ACCEL == st->plat_data.sec_slave_type) {
        ...
        else if(ACCEL_ID_KXTF9 == st->plat_data.sec_slave_id)
			 ...
			 const struct i2c_device_id *id)
{
    ...
    } else if (INV_ITG3500 != st->chip_type && INV_MPU3050 != st->chip_type) {
