...
#define DEF_GYRO_WAIT_TIME          (20)//(30) //note: 512 byte/12 byte ~= 42 (FIFO buf size maybe only 512 byte, accel + gyro = 6 byte)
...
static struct test_setup_t test_setup = {
	.gyro_sens   = 32768 / (GYRO_DPS_SCALE << DEF_SELFTEST_GYRO_FULL_SCALE),
...
int inv_get_silicon_rev_mpu6500(struct inv_gyro_state_s *st)
{
	...
	result = inv_i2c_read(st, st->reg->who_am_i/*REG_WHOAMI*/, 1, &whoami);
	printk("%s, whoami=0x%x\n", __func__, whoami);
	if (result)
		return result;
...
static int inv_check_accl_self_test_raw(struct inv_gyro_state_s *st, int *reg_avg)
{
...
#else
	fs = (ACCL_G_SCALE*1000UL << DEF_SELFTEST_ACCL_FULL_SCALE);
#endif
	g_z_sign = 1;
	ret_val = 0;
...
static int inv_check_3500_gyro_self_test_raw(struct inv_gyro_state_s *st, int *reg_avg)
{
	int i, ret_val=0;
...
static int inv_check_6050_gyro_self_test_raw(struct inv_gyro_state_s *st, int *reg_avg)
{
...
static int inv_do_test_mpu3050(struct inv_gyro_state_s *st, int *gyro_result)
{
	struct inv_reg_map_s *reg;
	...
	reg = st->reg;
	packet_size = 6;
	result = inv_i2c_single_write(st, reg->pwr_mgmt_1, 0x01);
	if (result)
		return result;
	...
	/* clear FIFO */
	result = inv_i2c_single_write(st, reg->user_ctrl, 0x02);
	if (result)
		return result;
	mdelay(15);
	/* setup parameters */
	//set lpf and fsr
	result = inv_i2c_single_write(st, reg->lpf, test_setup.lpf | (test_setup.fsr << GYRO_CONFIG_FSR_SHIFT));
	if (result)
		return result;
	...
	if (abs(packet_count - DEF_GYRO_PACKET_THRESH)
		...
		for (i = 0; i < packet_count; i++) {
			...
			gyro_result[0] += (short)((data[ind + 0] << 8) | data[ind + 1]);
			gyro_result[1] += (short)((data[ind + 2] << 8) | data[ind + 3]);
			gyro_result[2] += (short)((data[ind + 4] << 8) | data[ind + 5]);
		}
	}
	else {
		return -EAGAIN;
...
static void inv_recover_setting(struct inv_gyro_state_s *st)
{
	...
	{
		inv_i2c_single_write(st, reg->lpf, (st->chip_config.lpf) | (st->chip_config.fsr << GYRO_CONFIG_FSR_SHIFT));
	}
	else
	{
		inv_i2c_single_write(st, reg->gyro_config, st->chip_config.fsr << GYRO_CONFIG_FSR_SHIFT);
	...
	if (INV_ITG3500 != st->chip_type) {
		inv_i2c_single_write(st, reg->accl_config,
			(st->chip_config.accl_fs << ACCL_CONFIG_FSR_SHIFT)|0);
	}
...
int inv_self_test_mpu3050(struct inv_gyro_state_s *st,
		int *gyro_bias_regular)
{
	...
	test_times = 2;
	while (test_times > 0) {
		result = inv_do_test_mpu3050(st, gyro_bias_regular);
		if (result == -EAGAIN)
			test_times--;
		else
			test_times = 0;
	...
	if (result)
		return (0x00<<2) | (0x01<<1) | 0x00;
	else
		return (0x00<<2) | (0x01<<1) | 0x01;
}
		     ...
		     int *gyro_bias_regular)
{
	...
	if (st->chip_type == INV_ITG3500) {
		gyro_result = inv_check_3500_gyro_self_test(st, gyro_bias_regular, gyro_bias_st);
	        gyro_result |= inv_check_3500_gyro_self_test_raw(st, gyro_bias_regular);
		...
		gyro_result |= inv_check_6050_gyro_self_test_raw(st, gyro_bias_regular);
		gyro_result = !gyro_result;
	}
test_fail:
	inv_recover_setting(st);
...
int inv_hw_self_test_6500(struct inv_gyro_state_s *st,
		int *gyro_bias_regular)
{
	int result;
...
int inv_get_accl_bias(struct inv_gyro_state_s *st, int *accl_bias_regular)
{
	...
	accl_bias_regular[0] = accl_bias_regular[0] * st->chip_info.multi;
	accl_bias_regular[1] = accl_bias_regular[1] * st->chip_info.multi;
	accl_bias_regular[2] = accl_bias_regular[2] * st->chip_info.multi;
...
int inv_get_accl_bias_mpu3050(struct inv_gyro_state_s *st, int *accl_bias_regular)
{
	int i = 0;
	int result = -EAGAIN;
	int flg_suspended = 0;
	...
	accl_bias_regular[0] = accl_bias_regular[1] = accl_bias_regular[2] = 0;

	/* see if the slave is suspended */
	if( st->mpu_slave->get_mode!=NULL )
	{
		if( st->mpu_slave->get_mode(st) == 0 )
		{
			flg_suspended = 1;
		}
	}

	if( flg_suspended == 1 && st->mpu_slave->resume != NULL )
	{
	...
	for(i = 0; i < 75; i++ ) {
		...
		while( result && tries++ < 5 ) {
			if( st->mpu_slave->bypass_read != NULL )
				result = st->mpu_slave->bypass_read(st, data);
			mdelay(10);
		}
		...
		accel_data[0] = (unsigned short)data[0] | (unsigned short)data[1] << 8;
		accel_data[1] = (unsigned short)data[2] | (unsigned short)data[3] << 8;
		accel_data[2] = (unsigned short)data[4] | (unsigned short)data[5] << 8;

		accl_bias_regular[0] += accel_data[0];
		accl_bias_regular[1] += accel_data[1];
		accl_bias_regular[2] += accel_data[2];
	}

	accl_bias_regular[0] = accl_bias_regular[0] * DEF_GYRO_PRECISION / 4 /75;
	accl_bias_regular[1] = accl_bias_regular[1] * DEF_GYRO_PRECISION / 4 /75;
	accl_bias_regular[2] = accl_bias_regular[2] * DEF_GYRO_PRECISION / 4 /75;

	printk("%d, %d, %d\n", accl_bias_regular[0], accl_bias_regular[1], accl_bias_regular[2]);

	if( flg_suspended == 1 && st->mpu_slave->suspend != NULL )
	{
