...
#define I2C_HID_PROBE_PENDING	(1 << 3)
#define I2C_HID_CLOSED		(1 << 4)
...
#ifdef TP_WAKEUP
static bool pwr_sleep_suspend = 0;
#endif

static bool is_wake_enable = 0;
...
static const struct i2c_hid_cmd hid_output_report_cmd =	{ .length = 0 };
		...
		unsigned char *buf_recv, int data_len)
{
	...
	i2c_hid_dbg(ihid, "%s: args = %*ph\n", __func__, args_len, args);
	...
	i2c_hid_dbg(ihid, "%s: msg=%*ph msg_num=%d\n",__func__,
		sizeof(struct i2c_msg) * msg_num, msg, msg_num);
...
static void i2c_hid_get_input(struct i2c_hid *ihid)
{
	...
	if (test_bit(I2C_HID_STARTED, &ihid->flags)) {
		ret = hid_input_report(ihid->hid, HID_INPUT_REPORT, ihid->inbuf + 2,
				ret_size - 2, 1);
		if (ret < 0)
			dev_err(&ihid->client->dev, "hid_input_report failed: error = %d\n",
				ret);

	} else
		dev_err(&ihid->client->dev, "Got input but device not started\n");
...
static irqreturn_t i2c_hid_irq(int irq, void *dev_id)
{
	...
	if (test_bit(I2C_HID_CLOSED, &ihid->flags))
		return IRQ_HANDLED;

#ifdef TP_WAKEUP
       if (ihid->pwr_state == I2C_HID_PWR_SLEEP && pwr_sleep_suspend == 1) {
               dev_dbg(&ihid->client->dev, "%s: ## set power on.\n", __func__);
               ret = i2c_hid_set_power(ihid->client, I2C_HID_PWR_ON);
               if (ret)
                       dev_err(&ihid->client->dev, "%s: set power state failed.\n", __func__);
       }
#endif
...
static void i2c_hid_stop(struct hid_device *hid)
{
	...
	if (ihid) {
		i2c_hid_free_buffers(ihid);
		if (test_bit(I2C_HID_PROBE_PENDING, &ihid->flags))
			ihid->stop_in_probe = 1;
	}
}
...
static int i2c_hid_open(struct hid_device *hid)
{
	...
	if (!hid->open++) {
		...
		clear_bit(I2C_HID_CLOSED, &ihid->flags);
	}
done:
...
static void i2c_hid_close(struct hid_device *hid)
{
	...
	if (!--hid->open) {
		...
		set_bit(I2C_HID_CLOSED, &ihid->flags);
...
static int __devinit i2c_hid_fetch_hid_descriptor(struct i2c_hid *ihid)
{
	...
	i2c_hid_dbg(ihid, "wDataRegister = 0x%02x\n", ihid->hdesc.wDataRegister);
	i2c_hid_dbg(ihid, "wOutputRegister = 0x%02x\n", ihid->hdesc.wOutputRegister);
		...
		const struct i2c_device_id *dev_id)
{
	...
	set_bit(I2C_HID_PROBE_PENDING, &ihid->flags);
	clear_bit(I2C_HID_CLOSED, &ihid->flags);
	...
	if (ihid->stop_in_probe) {
		ret = -ENODEV;
		goto err_mem_free;
	}

	clear_bit(I2C_HID_PROBE_PENDING, &ihid->flags);
	return 0;
...
static int i2c_hid_suspend(struct device *dev)
{
	...
	if (device_may_wakeup(&client->dev) && bowser_is_lid_opened()) {
...
static int i2c_hid_resume(struct device *dev)
{
	...
	if (device_may_wakeup(&client->dev) && is_wake_enable) {
		disable_irq_wake(client->irq);
		is_wake_enable = 0;
	}
